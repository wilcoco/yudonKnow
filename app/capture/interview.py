...
from app.capture.instruments import BY_KEY, LADDER, slot_ko
...
_SYSTEM = """너는 은퇴를 앞둔 숙련 전문가에게서 암묵지를 캐내는 발굴 도우미다.
...
def next_question(
    ...
    *,
    ...
    if gap_question:
        return Question(
            text=f"후배가 이걸 물었는데 분신이 답하지 못했습니다.\n\n"
                 f"「{gap_question}」\n\n어떻게 보십니까?",
            rung="gap",
    ...
    if instrument != LADDER and not history:
        ...
        if tool:
            return Question(text=tool.opener, instrument=instrument, rung="opener")
    ...
    rung = LADDER_RUNGS[min(len(history), len(LADDER_RUNGS) - 1)]
    fallback_text = SLOT_QUESTIONS[target] if target else rung[1]

    if not history:
        return Question(text=LADDER_RUNGS[0][1], rung="recall", instrument=instrument)

    prompt = _build_probe_prompt(history, target, instrument)
    try:
        text = llm.answer(_SYSTEM, prompt).strip()
...
def _build_probe_prompt(
    history: list[tuple[str, str]], target: str, instrument: str
) -> str:
    lines = ["지금까지의 대화:"]
    ...
    if tool and instrument != LADDER:
        lines.append(f"\n지금 쓰는 연장: {tool.name} — {tool.pitch}")
    if target:
        lines.append(
            f"\n지금 비어 있는 칸: **{slot_ko(target)}**. 이 칸을 채우는 질문 하나만 만들어라."
        )
    else:
        lines.append("\n사다리 다음 단으로 파고드는 질문 하나만 만들어라.")
    lines.append("질문 문장만 출력해라. 머리말·설명 금지.")
    return "\n".join(lines)
...
def wrong_answer(llm: BaseLLM, topic: str, domain: str = "") -> dict[str, str]:
...
def capture(llm: BaseLLM, history: list[tuple[str, str]]) -> CardDraft:
    """대화 → 카드 초안. 실패해도 **원본은 증발하지 않는다.**"""
    transcript = "\n".join(f"질문: {q}\n전문가: {a}" for q, a in history)
    try:
...
def slot_report(card: Card) -> dict[str, Any]:
    """인터뷰 화면 오른쪽 패널 — 어느 칸이 비었고 다음에 뭘 물을지."""
    return {
        "filled": [slot_ko(s) for s in card.filled],
        "missing": [slot_ko(s) for s in card.missing],
        "completeness": round(card.completeness, 2),
        "citable": card.citable(),
        "blocker": "신호" if not card.cues else "",
        "next_question": SLOT_QUESTIONS.get(card.missing[0], "") if card.missing else "",
        "slots": [{"key": s, "ko": slot_ko(s), "filled": s in card.filled} for s in SLOTS],
    }
